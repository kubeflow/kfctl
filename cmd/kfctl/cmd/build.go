...
var buildCmd = &cobra.Command{
	...
	RunE: func(cmd *cobra.Command, args []string) error {
		...
		if applyCfg.GetBool(string(kftypes.DUMP)) == true {
...
func init() {
	...
	// dump flag
	applyCmd.Flags().BoolP(string(kftypes.DUMP), "d", false,
		string(kftypes.DUMP)+" manifests to stdouit instead of applying, default is false")
	bindErr = applyCfg.BindPFlag(string(kftypes.DUMP), applyCmd.Flags().Lookup(string(kftypes.DUMP)))
	if bindErr != nil {
		log.Errorf("Couldn't set flag --%v: %v", string(kftypes.VERBOSE), bindErr)
		return
	}
